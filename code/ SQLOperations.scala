...
object SQLOperations {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Setup: prepare results/sql_results.csv for writing query outputs
    new java.io.File("results").mkdirs()
    val csvFile = new java.io.PrintWriter(new java.io.FileWriter("results/sql_results.csv", false))

    // Helper: append a labeled section + DataFrame rows to the CSV file
    def appendToCSV(label: String, df: org.apache.spark.sql.DataFrame): Unit = {
      csvFile.println(s"# $label")
      csvFile.println(df.columns.mkString(","))
      df.collect().foreach { row =>
        csvFile.println((0 until row.size).map(i => Option(row(i)).map(_.toString).getOrElse("")).mkString(","))
      }
      csvFile.println()
    }
    ...
    appendToCSV("Stations with the Largest Imbalance Between Outgoing and Incoming Trips", stationImbalanceResult)
    ...
    appendToCSV("Top Used Stations with Electric and Classic Bike Percentages", bikePreferenceResult)
    ...
    appendToCSV("Most Stable Demand Hours Across the Week", stableHoursResult)
    ...
    appendToCSV("Least Stable Demand Hours Across the Week", unstableHoursResult)
    ...
    appendToCSV("Most Used Start-to-End Routes During Peak Hours", peakRoutesResult)
    ...
    appendToCSV("Trip-Duration Statistics by Rider Type", riderTypeStatsResult)

    // Query 6: Electric vs Classic Bike Preference by Day of Week
    // Uses: GROUP BY, SUM aggregation, ROUND, ORDER BY

    val bikePreferenceByDayQuery =
      """
        |SELECT
        |  day_of_week,
        |  SUM(classic_trips) AS total_classic_trips,
        |  SUM(electric_trips) AS total_electric_trips,
        |  SUM(classic_trips + electric_trips) AS total_trips,
        |  ROUND(SUM(electric_trips) * 100.0 /
        |        SUM(classic_trips + electric_trips), 2) AS electric_share_percent,
        |  ROUND(SUM(classic_trips) * 100.0 /
        |        SUM(classic_trips + electric_trips), 2) AS classic_share_percent
        |FROM trips
        |GROUP BY day_of_week
        |ORDER BY day_of_week
        |""".stripMargin

    val bikePreferenceByDayResult = spark.sql(bikePreferenceByDayQuery)

    println()
    println("============================================================")
    println("Electric vs Classic Bike Preference by Day of Week")
    println("============================================================")
    bikePreferenceByDayResult.show(7, false)
    appendToCSV("Electric vs Classic Bike Preference by Day of Week", bikePreferenceByDayResult)
