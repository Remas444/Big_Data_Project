...
// -----------------------------------
// Helper: cast schema after reading reducedData.csv
// IMPORTANT: reducedData.csv is saved as CSV -> types become STRING when read back
// -----------------------------------
def castReducedFromCsv(df: DataFrame): DataFrame = {
  df
    .withColumn("hour_bucket", to_timestamp(col("hour_bucket")))
    .withColumn("classic_trips", col("classic_trips").cast("long"))
    .withColumn("electric_trips", col("electric_trips").cast("long"))
    .withColumn("avg_duration_min", col("avg_duration_min").cast("double"))
    .withColumn("start_station_name", trim(col("start_station_name")))
    .withColumn("end_station_name", trim(col("end_station_name")))
    .withColumn("member_casual", lower(trim(col("member_casual"))))
}
...
// =========================================================
// (D) TRANSFORMATION STEP -> reads reducedData.csv -> writes transformedData.csv
// =========================================================
val reducedFromFileRaw = spark.read
  .option("header", "true")
  .option("mode", "PERMISSIVE")
  .csv("data/processed/reducedData.csv")

val tRead = reducedFromFileRaw.count()
println(s"[COUNT] Transform-Read (reducedData.csv rows): $tRead")

// Cast back to correct types (important!)
val reducedFromFile = castReducedFromCsv(reducedFromFileRaw)
val tCast = reducedFromFile.count()
println(s"[COUNT] Transform-AfterCast (rows): $tCast")

// ---------------------------------------------------------
// SNAPSHOT A: BEFORE transformation (aligned, short headers)
// ---------------------------------------------------------
val beforeSnap = reducedFromFile.select(
  col("hour_bucket").alias("hour_bucket"),
  col("start_station_name").alias("start_station"),
  col("end_station_name").alias("end_station"),
  col("member_casual").alias("type"),
  col("classic_trips").alias("classic"),
  col("electric_trips").alias("electric"),
  col("avg_duration_min").alias("avg_min")
)

println("\n[SNAPSHOT] Before Transformation (10 rows) - reducedData after cast:")
println("[COLUMNS]")
println(beforeSnap.columns.mkString(" | "))
beforeSnap.show(10, truncate = false)

// (T-1) hour_bucket -> date + hour + day_of_week + is_weekend, then DROP hour_bucket
val step1 = reducedFromFile
  .withColumn("date", to_date(col("hour_bucket")))
  .withColumn("hour", hour(col("hour_bucket")))
  .withColumn("day_of_week", dayofweek(col("hour_bucket")).cast("int")) // 1=Sun ... 7=Sat
  .withColumn("is_weekend", when(col("day_of_week").isin(1, 7), 1).otherwise(0)) // Sat/Sun weekend
  .drop("hour_bucket")

// (T-2) Encode categorical columns as NEW columns
val startIndexer = new org.apache.spark.ml.feature.StringIndexer()
  .setInputCol("start_station_name")
  .setOutputCol("start_station_name_idx_tmp")
  .setHandleInvalid("keep")

val endIndexer = new org.apache.spark.ml.feature.StringIndexer()
  .setInputCol("end_station_name")
  .setOutputCol("end_station_name_idx_tmp")
  .setHandleInvalid("keep")

val memberIndexer = new org.apache.spark.ml.feature.StringIndexer()
  .setInputCol("member_casual")
  .setOutputCol("member_casual_idx_tmp")
  .setHandleInvalid("keep")

val startModel = startIndexer.fit(step1)
val step2a = startModel.transform(step1)
  .withColumn("start_station_name_idx", col("start_station_name_idx_tmp").cast("int"))
  .drop("start_station_name_idx_tmp")

val endModel = endIndexer.fit(step2a)
val step2b = endModel.transform(step2a)
  .withColumn("end_station_name_idx", col("end_station_name_idx_tmp").cast("int"))
  .drop("end_station_name_idx_tmp")

val memberModel = memberIndexer.fit(step2b)
val step2 = memberModel.transform(step2b)
  .withColumn("member_casual_idx", col("member_casual_idx_tmp").cast("int"))
  .drop("member_casual_idx_tmp")

// (T-3) Final order (your CSV order)
val transformedFinal = step2.select(
  col("date"),
  col("hour"),
...
println(s"[COUNT] Transform-Final (rows): ${transformedFinal.count()}")

// ---------------------------------------------------------
// SNAPSHOT B: AFTER transformation (aligned, short headers)
// (same order as CSV but readable + aligned like your terminal screenshot)
// ---------------------------------------------------------
val afterSnap = transformedFinal.select(
  col("date").alias("date"),
  col("hour").alias("hr"),

  col("start_station_name").alias("start_station"),
  col("start_station_name_idx").alias("start_idx"),

  col("end_station_name").alias("end_station"),
  col("end_station_name_idx").alias("end_idx"),

  col("member_casual").alias("type"),
  col("member_casual_idx").alias("type_idx"),
  ...
  col("is_weekend").alias("weekend")
)

println("\n[SNAPSHOT] After Transformation (10 rows) - final transformedData:")
println("[COLUMNS]")
println(afterSnap.columns.mkString(" | "))
afterSnap.show(10, truncate = false)

// Save final transformed
writeSingleCsv(
  transformedFinal,
  tmpDir = "data/processed/transformedData_tmp",
  finalFile = "data/processed/transformedData.csv"
)
println("✅ Saved: data/processed/transformedData.csv")
    spark.stop()
  }
}
